...
| 📱 Mobile Agent | `mobile_agent` | [docker-android](https://github.com/budtmo/docker-android) | 基于 OpenSandbox + docker-android 的 Android GUI 操控轨迹合成，VLM 驱动截图→推理→ADB 动作循环 |
...
# Android 模拟器镜像（Mobile Agent 场景, 可选）
docker pull budtmo/docker-android:emulator_14.0

# 国内加速: docker-android 托管在 Docker Hub, 配置镜像加速即可
...
基于 OpenSandbox + [docker-android](https://github.com/budtmo/docker-android)，在 OpenSandbox 管理的 Android 模拟器容器中运行 VLM 驱动的 GUI Agent。Pipeline 为：场景加载 → OpenSandbox 创建 Android 沙箱 → 截图+UI树采集(ADB) → VLM 推理与动作决策 → ADB 执行动作 → 循环 → 5 维质量评估 → 数据集导出。
...
- **沙箱镜像不同** — 使用 `budtmo/docker-android`（Android AVD），但同样由 OpenSandbox Server 统一管理
...
| `MOBILE_SANDBOX_IMAGE` | ❌ | Android 模拟器镜像，默认 `budtmo/docker-android:emulator_14.0` |
...
| Disk | 40 GB+ | 沙箱镜像需要存储空间（docker-android 镜像约 5 GB） |
...
```
docker-android 镜像托管在 Docker Hub，配置镜像加速即可。
...
**Q: 不同场景对沙箱环境有什么要求？**
大多数场景共用 OpenSandbox 镜像（`opensandbox/code-interpreter:v1.0.2`），差异体现在 Agent 的系统提示和任务描述上。Mobile Agent 场景同样由 OpenSandbox 管理，但使用 Android 模拟器镜像（`budtmo/docker-android`），通过 ADB 进行 GUI 操控。

**Q: Mobile Agent 在 macOS 上运行报 KVM 错误？**
docker-android 内部运行 Android 模拟器，需要虚拟化支持。Apple Silicon 用户需确保 Docker Desktop 启用了 Rosetta 2，或使用 Colima：
```bash
colima start --vm-type=vz --vz-rosetta --memory 8 --cpu 4
```

**Q: OpenSandbox Server 不可达时能否使用 Mobile Agent？**
可以。Server 不可达时自动切换到 Mock 模式，Pipeline 逻辑正常运行但不会真正操控 Android 设备，适合开发调试 Agent Prompt 和验证 Pipeline 流程。

**Q: 并发多少个沙箱合适？**
16 GB 内存的机器建议最多同时运行 3 个 OpenSandbox 沙箱，可在 Web UI 中调整并发数。docker-android 资源消耗更大（约 4GB 内存），建议单独运行。
...
- [x] ~~GUI 操作场景专用桌面环境镜像~~ → Mobile Agent (OpenSandbox + docker-android)
...
| 移动端 | [docker-android](https://github.com/budtmo/docker-android) · Android AVD · ADB · uiautomator2 |
