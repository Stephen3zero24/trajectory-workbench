...
- **真实环境交互** — Agent 在 OpenSandbox / AgentScope Runtime 沙箱中执行真实命令与 GUI 操控，轨迹数据源于闭环交互而非凭空生成
...
| 📱 Mobile Agent | `mobile_agent` | [AgentScope Runtime](https://github.com/agentscope-ai/agentscope-runtime) | 基于 AgentScope MobileSandbox 的 Android GUI 操控轨迹合成，VLM 驱动截图→推理→动作循环 |
...
│   ├── sandbox_runner.py     #   AgentScope MobileSandbox 生命周期管理
...
# AgentScope MobileSandbox 镜像（Mobile Agent 场景, 可选）
...
# Mobile Agent (无需 OpenSandbox, 直接使用 AgentScope MobileSandbox)
...
基于 [AgentScope Runtime](https://github.com/agentscope-ai/agentscope-runtime) 的 MobileSandbox，在 Docker 化的 Android 模拟器中运行 VLM 驱动的 GUI Agent。Pipeline 为：场景加载 → MobileSandbox 启动 → 截图+UI树采集 → VLM 推理与动作决策 → 动作执行 → 循环 → 5 维质量评估 → 数据集导出。

与其他引擎的核心区别：

- **Observation 是视觉的** — 通过截图 + uiautomator dump 的 UI hierarchy 理解界面，而非文本工具返回值
- **Action 是空间操控** — tap(坐标)、swipe(轨迹)、input_text、key_event 等 7 种 GUI 动作
- **沙箱是 Android 模拟器** — 使用 AgentScope MobileSandbox（Docker 容器内 Android AVD），独立于 OpenSandbox
- **Review 维度不同** — UI 理解、动作准确性、推理清晰度、完成度、效率
- **内置 10 个场景** — 覆盖系统设置、闹钟、联系人、计算器、跨应用多步操作等

详细文档见 [`mobile_agent/README.md`](mobile_agent/README.md)。
...
| `AGENTSCOPE_MOBILE_IMAGE` | ❌ | MobileSandbox 镜像，默认 `agentscope/runtime-sandbox-mobile:latest` |
...
| Disk | 40 GB+ | 沙箱镜像需要存储空间（MobileSandbox 镜像约 5 GB） |
...
```
AgentScope 镜像也可从阿里云 ACR 拉取（见[第四步](#第四步拉取沙箱镜像)）。
...
**Q: 不同场景对沙箱环境有什么要求？**
大多数场景共用 OpenSandbox 镜像（`opensandbox/code-interpreter:v1.0.2`），差异体现在 Agent 的系统提示和任务描述上。Mobile Agent 场景使用独立的 AgentScope MobileSandbox 镜像（`agentscope/runtime-sandbox-mobile`），不依赖 OpenSandbox。

**Q: Mobile Agent 在 macOS 上运行报 KVM 错误？**
MobileSandbox 内部运行 Android 模拟器，需要虚拟化支持。Apple Silicon 用户需确保 Docker Desktop 启用了 Rosetta 2，或使用 Colima：
```bash
colima start --vm-type=vz --vz-rosetta --memory 8 --cpu 4
```

**Q: 未安装 agentscope-runtime 能否使用 Mobile Agent？**
可以。未安装时自动切换到 Mock 模式，Pipeline 逻辑正常运行但不会真正操控 Android 设备，适合开发调试 Agent Prompt 和验证 Pipeline 流程。

**Q: 并发多少个沙箱合适？**
16 GB 内存的机器建议最多同时运行 3 个 OpenSandbox 沙箱，可在 Web UI 中调整并发数。MobileSandbox 资源消耗更大，建议单独运行。
...
- [x] ~~GUI 操作场景专用桌面环境镜像~~ → Mobile Agent (AgentScope MobileSandbox)
...
| 层级 | 技术 |
|------|------|
| 沙箱平台 | [OpenSandbox](https://github.com/alibaba/OpenSandbox) (Alibaba)、[AgentScope Runtime](https://github.com/agentscope-ai/agentscope-runtime) (Alibaba) |
...
| 移动端 | AgentScope MobileSandbox · Android AVD · uiautomator2 |
...
- [MobileWorld: Benchmarking Autonomous Mobile Agents in Real-World Scenarios](https://arxiv.org/abs/2512.19432)
