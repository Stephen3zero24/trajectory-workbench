...
// ─── Phase: 执行监控 ─────────────────────────────────────────────────────────
function PhaseMonitor({ taskId, onReview, onComplete }) {
  ...
  useEffect(() => {
    const poll = setInterval(async () => {
      try {
        ...
        const e = await api(`/tasks/${taskId}/events?since=${evtIdx.current}`);
  ...
  const statusLabel = { created: "初始化", executing: "沙箱执行中", reviewing: "评估中", waiting_approval: "等待审批", completed: "已完成", failed: "失败" };
  const statusColor = { executing: "accent", reviewing: "purple", waiting_approval: "amber", completed: "green", failed: "red" };
    ...
    <div style={{ maxWidth: 800, margin: "0 auto" }}>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <Title>实时日志</Title>
          {task && <Badge color="purple">迭代 {(task.current_iteration || 0) + 1} / {task.max_iterations}</Badge>}
        </div>
...
// ─── Phase: 审批评估 ─────────────────────────────────────────────────────────
function PhaseReview({ task, taskId, onIterate, onExport, onTaskUpdate }) {
  const [selectedOptions, setSelectedOptions] = useState({});
  const [expandedStep, setExpandedStep] = useState(null);
  const [approving, setApproving] = useState(false);

  if (!task || !task.iterations || task.iterations.length === 0) return null;
  ...
  const passThreshold = score >= (task.quality_threshold || 0.8);
        ...
        <Card>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {Object.entries(dims).map(([k, v]) => {
              const labels = { tool_usage: "工具调用", reasoning: "推理质量", error_handling: "错误处理", completeness: "任务完成度" };
...
// ─── Phase: 导出 ─────────────────────────────────────────────────────────────
function PhaseExport({ task, taskId }) {
  const [exportResult, setExportResult] = useState(null);

  useEffect(() => {
    api(`/tasks/${taskId}/export`, { method: "POST" }).then(setExportResult);
  }, [taskId]);
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center" }}>
      ...
      <p style={{ color: C.textMuted, fontSize: 13, margin: "8px 0 28px" }}>
        经过 {iterations.length} 轮迭代 · 最佳质量: {exportResult ? (exportResult.best_score * 100).toFixed(0) : "--"}/100
      </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 28 }}>
        {["SFT 格式", "DPO 格式", "RLHF 格式"].map((fmt, i) => (
          <Card key={i} style={{ textAlign: "center", padding: 20 }}>
            ...
            <div style={{ fontSize: 11, color: C.textMuted, marginTop: 4 }}>{exportResult?.file || "..."}</div>
          </Card>
        ))}
      </div>
...
export default function App() {
  ...
  const handleComplete = (task) => {
    setTaskData(task);
    if (task.status === "completed") setStage("review");
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }}>
      ...
      <div style={{ padding: "32px 28px", maxWidth: 960, margin: "0 auto" }}>
        ...
        {stage === "monitor" && taskId && <PhaseMonitor taskId={taskId} onReview={handleReview} onComplete={handleComplete} />}
        ...
        {stage === "export" && <PhaseExport task={taskData} taskId={taskId} />}
      </div>
