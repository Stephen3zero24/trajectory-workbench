...
class TaskState(BaseModel):
    ...
    max_iterations: int = 3
    quality_threshold: float = 0.80
    iterations: list = Field(default_factory=list)  # 每轮的轨迹+评估结果
...
def apply_auto_fixes_to_config(config: dict, suggestions: list) -> dict:
    """自动应用 auto 级别的修改"""
    applied = []
...
app = FastAPI(title="轨迹合成工作台 API", version="1.0.0", lifespan=lifespan)
...
async def export_dataset(task_id: str):
    ...
    export_data = {
        ...
        "best_score": best_score,
        "quality_progression": [
            it.get("review", {}).get("overall_score", 0)
